 ...
 *
 * ★ v0.6 보정A 확정값 (2026-09-16, `npm run headless -- --growth --policy greedy --seed 1..80` 실측) ★
 * v0.6 에서 스킬 위력이 크게 올라(skills.ts 계수 메모) 스킬을 3+2개 갖춘 플레이어가 스킬 1+floor(day/4)개인 생성 상대를
 * 압도했다 (v0.5 표 그대로면 탐욕 정책 승률 65~68%). 이 표를 2~5·7~10일차 +8% 수준으로 올린 뒤 일차별로 다듬어
 * 80시드 800판 승률 49.0% (시드 1~40: 47.0%, 41~80: 51.0%, 무승부 1.0%) 에 맞췄다. 민감도: 표 1% ≈ 승률 2.5~3%p.
...
export const OPPONENT_POWER_BY_DAY: readonly number[] = [
  3860, // 1일차 (첫날은 양쪽 다 갓 만든 팀이라 동급. 플레이어는 선택지 3장만 적용된 상태라 목표를 약간 낮게 잡는다)
  4470, // 2일차
  4790, // 3일차
  4720, // 4일차
  4740, // 5일차
  4460, // 6일차 — 여기서 상대가 전원 세부 직업으로 분화한다. 분화는 점수보다 실전 값어치가 커서
  4700, // 7일차    같은 승률을 유지하려면 점수 목표를 오히려 한 단계 낮춰야 한다 (5일차 > 6일차)
  4780, // 8일차
  4830, // 9일차
  4880, // 10일차
];
